...
def _get_build_log(e):
    log = str(getattr(e, 'what', '')) or str(getattr(e, 'log', '')) or (e.args[1] if len(e.args) > 1 and isinstance(e.args[1], (str, bytes)) else '')
    return log.decode(errors='ignore') if isinstance(log, bytes) else log.strip()
...
def brute_force_on_device(alg_name, target_hash, length, charset_bytes, batch_mult):
    ...
    kernel_src = kernel_code % {"length": length, "base": len(charset_bytes), "charset_data_str": charset_init}

    try: prg = cl.Program(ctx, kernel_src).build()
    except cl.RuntimeError as e:
        print(f"OpenCL Kernel Compile Err (len {length}): {e.code}\nLog: {_get_build_log(e) or e.what()}"); return None
    ...
    for offset in range(0, max_combs, cand_launch):
        ...
        try:
            ...
            print(f"OpenCL Launch Err (len {length}): {e.code}\nDetails: {_get_build_log(e) or e.what()}"); return None
...
def apply_leetspeak(word):
    """Applies a few common leetspeak substitutions."""
        ...
        with open(wordlist_file, 'r', encoding='utf-8', errors='ignore') as f:
            for line_idx, line in enumerate(f):
                ...
                if base_words_checked % progress_interval_base == 0:
                    ...
                    print(f"\r[*] Rules: Base words processed: {base_words_checked:,}. Mutations tried: {mutations_tried:,}. Time: {elapsed_rules:.2f}s", end="")
                ...
                leet_variants_for_base = set()
                for bw in base_candidates:
                    leet_variants_for_base.update(apply_leetspeak(bw))
                base_candidates.update(leet_variants_for_base)

                for candidate_root in base_candidates:
                    for suffix in suffixes:
                        ...
                        if computed_hash == target_hash_hex_str.lower():
                            ...
                            print(f"    Base words processed: {base_words_checked:,}. Mutations tried: {mutations_tried:,}")
            ...
            if current_len <= KERNEL_MAX_STRING_LENGTH:
                if not opencl_init:
                    try:
                        ...
                        setup_opencl(device_type_pref); opencl_init = True
                    except RuntimeError as e:
                        print(f"OpenCL Setup Error: {e}. Will try CPU if length > {KERNEL_MAX_STRING_LENGTH}."); opencl_init = False 
                        if current_len <= KERNEL_MAX_STRING_LENGTH: print("Cannot use GPU. Stopping."); return False
                if opencl_init:
                    result = brute_force_on_device(algorithm_name, target_hash_to_find, current_len, charset_bytes, batch_size_multiplier)
            else: 
                if not cpu_msg_shown:
                    print(f"\n--- Switching to CPU for lengths > {KERNEL_MAX_STRING_LENGTH} (slower) ---"); cpu_msg_shown = True
                result = brute_force_on_cpu_for_length(algorithm_name, target_hash_to_find, current_len, charset_bytes, overall_start_time)

            if result is None: print(f"--- Error at length {current_len}. Stopping."); return False
            if isinstance(result, str): return True 

            current_len += 1

            if user_max_len is None and current_len > KERNEL_MAX_STRING_LENGTH + 10 and current_len > KERNEL_MAX_STRING_LENGTH :
